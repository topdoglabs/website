...
The branch preview deployed successfully in [PR #1](https://github.com/topdoglabs/website/pull/1). Authenticated browser checks passed for direct app navigation and refresh, route metadata, missing-page rendering, 320px layouts, and screenshot-dialog Escape/focus restoration. The support endpoint accepts the exact deployment and branch-preview origins; a regression test covers rejection of unrelated project origins. One controlled test email was accepted by the provider through the preview form; inbox receipt is awaiting confirmation.

The shared Vercel WAF rule `Support form rate limit` was published: POST `/api/send-email`, five requests per IP per 600-second fixed window, then HTTP 429. Production checks with empty payloads returned HTTP 400 for the first five requests and HTTP 429 for the sixth, without sending mail. Website code remains on the preview branch pending release; the firewall rule is already live. See README for the rule identifier.

Final automated verification: production build and catalog validation passed; all 16 tests passed; `npm audit` reported zero vulnerabilities after dependency updates. Responsive screenshots total 3.85 MB across both sizes versus 47.16 MB of originals.
